...
// Devuelve los últimos 12 meses terminando en el mes actual.
// Ej: si hoy es Mayo 2026 -> Jun 2025 ... May 2026
// Se recalcula automáticamente cada mes.
// ============================================================
const MES_LABELS = ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];

// Pesos estacionales por categoría e índice de mes (0=Ene ... 11=Dic) para el mercado argentino.
const SEASONAL_WEIGHTS = {
        'tecnologia': [85,80,78,75,70,75,80,85,88,90,100,95],
        'hogar':      [90,85,82,80,82,85,88,90,85,82,90,95],
        'deportes':   [95,85,75,70,65,70,75,80,85,90,95,100],
        'moda':       [85,75,72,75,80,82,85,90,82,80,95,100],
        'mascotas':   [88,85,82,78,72,75,78,80,82,85,90,90],
        'bebe':       [95,88,80,75,72,75,80,85,82,78,82,85],
        'default':    [82,78,76,75,75,78,80,82,82,80,85,85]
};

function getRollingMonths() {
        const now = new Date();
        const months = [];
        // 11 meses atrás hasta el actual = ventana de 12 meses
  for (let i = 11; i >= 0; i--) {
            ...
            months.push({
                        ...
                        year: d.getFullYear(),
                        label: `${MES_LABELS[d.getMonth()]} ${String(d.getFullYear()).slice(2)}`
            });
  }
        return months;
}
...
function detectCategory(product) {
        ...
        for (const [cat, keywords] of Object.entries(categoryKeywords)) {
                  if (keywords.some(kw => productLower.includes(kw))) return cat;
        }
        return 'default';
}
...
async function getDemandaData(product) {
        try {
          ...
          // ====== VENTANA DINÁMICA DE 12 MESES (rolling) ======
          const detectedCat = detectCategory(product);
                  const weights = SEASONAL_WEIGHTS[detectedCat] || SEASONAL_WEIGHTS['default'];
                  const rolling = getRollingMonths();
                  const monthlyData = rolling.map(m => {
                              const w = weights[m.monthIndex] / 100;
                              const noise = (Math.random() * 5 - 2);
                              return {
                                            mes: m.mes,
                                            label: m.label,
                                            year: m.year,
                                            valor: Math.round(Math.max(10, Math.min(100, demandaScore * w + noise)))
                              };
                  });
          ...
          const firstHalf = monthlyData.slice(0,6).reduce((a,b)=>a+b.valor,0)/6;
                  ...
                  if (secondHalf > firstHalf * 1.15) temporalidad = 'pico reciente (últimos 6 meses)';
                  else if (secondHalf < firstHalf * 0.85) temporalidad = 'pico en la primera mitad del período';
          ...
          const rangoLabel = `${rolling[0].label} - ${rolling[11].label}`;
          ...
          return {
                      ...
                      descripcion: `"${product}" tiene ${total.toLocaleString('es-AR')} publicaciones activas en MeLi Argentina. Promedio de ${Math.round(avgSold)} ventas/unidad. Tendencias ${rangoLabel}.`,
                      tags, detectedCategory: detectedCat, realData: true
          };
        } catch (err) {
...
async function getMeliCompetitionData(product) {
  ...
  const topResults = resultsRel.slice(0,5).map((r,i) => {
            ...
            else if (rep.level_id==='5_gold'||rep.level_id==='4_gold') { repLabel='MercadoLider'; repClass='rep-gold'; }
            else if (rep.transactions?.total>100) { repLabel='Seller activo'; repClass='rep-green'; }
            return {
                        rank:i+1, name:r.title||'Sin titulo', price:r.price||0,
                        ...
                        reputation:repLabel, repClass, sellerNick:seller.nickname||'Anonimo',
  ...
  const newCount = allResults.filter(r=>r.condition==='new').length;
        ...
        const descripcion = `${total.toLocaleString('es-AR')} publicaciones activas. ${newCount} productos nuevos (${Math.round((newCount/allResults.length)*100)}%). ${oficialCount>0?oficialCount+' tiendas oficiales. ':''}${freeShipCount} con envio gratis. Rango: ARS ${precioMinARS.toLocaleString('es-AR')} - ${precioMaxARS.toLocaleString('es-AR')}.`;
...
// ============================================================
// FALLBACK por categoria (con ventana dinamica de 12 meses)
// ============================================================
function getCategoryTrendsFallback(product) {
        ...
        const weights = SEASONAL_WEIGHTS[detectedCategory] || SEASONAL_WEIGHTS['default'];
        const rolling = getRollingMonths();
        const monthlyData = rolling.map(m => ({
                  mes: m.mes,
                  label: m.label,
                  year: m.year,
                  valor: Math.round(Math.max(20, Math.min(100, weights[m.monthIndex] + (Math.random()*6-3))))
        }));
        const rangoLabel = `${rolling[0].label} - ${rolling[11].label}`;
        return {
                  ...
                  monthlyData, rangoFechas: rangoLabel,
                  tags: ['Argentina','Estimado por categoria'], detectedCategory, realData: false,
                  descripcion: `Datos estimados por categoria (${rangoLabel}). Configura variables MeLi para datos en tiempo real.`
        };
}
                  ...
                  try {
                              const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
                                            ...
                                            body: JSON.stringify({
                                                            ...
                                                            messages: [
                                                                  { role:'system', content:'Sos un analista experto en importaciones China-Argentina. Respondes SOLO con JSON valido sin markdown: {"scoreTotal":0-100,"scoresDemanda":0-100,"scoresCompetencia":0-100,"scoresMargen":0-100,"scoresRegulatorio":0-100,"labelDemanda":"string","labelCompetencia":"string","labelMargen":"string","labelRegulatorio":"string","veredicto":"VIABLE|VIABLE CON CONDICIONES|NO RECOMENDADO","veredictoTexto":"2-3 oraciones en espanol rioplatense","analisisCompleto":"max 350 palabras en espanol rioplatense"}' },
...
function getLocalAnalysis(prompt) {
        return {
                  ...
                  veredictoTexto:'Configura GROQ_API_KEY para analisis IA completo. Los datos de precios son 100% reales de MercadoLibre.',
                  analisisCompleto:'Para analisis detallado con IA, configura GROQ_API_KEY en Railway. Groq es gratuito en groq.com.'
        };
}
...
function getDefaultProducts(nicho) {
        return [
              ...
              { nombre:'Cargador Inalambrico 15W', score:76, margen:50, demanda:'alta', riesgo:'bajo', ticket:'$18.000', justificacion:'Bajo costo FOB, alta rotacion.', topPick:false },
              ...
              { nombre:'Soporte Auto Magnetico', score:68, margen:48, demanda:'media', riesgo:'bajo', ticket:'$12.000', justificacion:'Alta rotacion en canales digitales.', topPick:false }
                ];
}
...
app.post('/api/chat', async (req, res) => {
        ...
        const GROQ_API_KEY = process.env.GROQ_API_KEY;
        if (!GROQ_API_KEY) return res.json({ reply:'Configura GROQ_API_KEY para activar el consultor IA.' });
        try {
                  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
                              method:'POST',
                              headers:{ 'Authorization':`Bearer ${GROQ_API_KEY}`, 'Content-Type':'application/json' },
        ...
        } catch(err) { res.json({ reply:'Error de conexion.' }); }
});
...
app.listen(PORT, async () => {
        ...
        const r = getRollingMonths();
        console.log(`Ventana Google Trends activa: ${r[0].label} - ${r[11].label}`);
});
