...
function meliHeaders() {
    const token = getMeliToken();
    const headers = { 'Content-Type': 'application/json', 'User-Agent': 'ProductFinderIA/1.0' };
...
async function getDemandaData(product) {
    try {
          const token = getMeliToken();
          if (!token) return getCategoryTrendsFallback(product);

      const query = encodeURIComponent(product);
          const response = await fetch(`${MELI_BASE}/sites/MLA/search?q=${query}&limit=50&sort=relevance`, { headers: meliHeaders() });

      if (!response.ok) return getCategoryTrendsFallback(product);
...
// ============================================================
// COMPETENCIA REAL desde MeLi con token
// ============================================================
async function getMeliCompetitionData(product) {
    const token = getMeliToken();
    if (!token) throw new Error('MELI_ACCESS_TOKEN no configurado.');

  const searchQuery = encodeURIComponent(product);

  const [resAsc, resRel] = await Promise.all([
        fetch(`${MELI_BASE}/sites/MLA/search?q=${searchQuery}&limit=50&sort=price_asc`, { headers: meliHeaders() }),
        fetch(`${MELI_BASE}/sites/MLA/search?q=${searchQuery}&limit=50&sort=relevance`, { headers: meliHeaders() })
      ]);
  ...
  let categoryName = null;
    ...
    if (catId) {
          try {
                  const catRes = await fetch(`${MELI_BASE}/categories/${catId}`, { headers: meliHeaders() });
...
function getCategoryTrendsFallback(product) {
    ...
    return {
          ...
          descripcion:`Datos estimados por categoría. Configurá MELI_ACCESS_TOKEN para datos en tiempo real.`
    };
}

// ============================================================
// ANALISIS IA con Groq
// ============================================================
...
app.post('/api/analyze', async (req, res) => {
    ...
    try {
          ...
          const prompt = `Generá 5 productos recomendados para importar desde China a Argentina con: capital=${capital}, experiencia=${experiencia}, canal=${canal}, nicho=${nicho}, riesgo=${riesgo}. Respondé SOLO con JSON: {"products":[{"nombre":"string","score":0-100,"margen":0-100,"demanda":"alta|media|baja","riesgo":"alto|medio|bajo","ticket":"$XX.XXX","justificacion":"string","topPick":bool}]}`;
          const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
                  method:'POST',
                  headers:{ 'Authorization':`Bearer ${GROQ_API_KEY}`, 'Content-Type':'application/json' },
...
function getDefaultProducts(nicho) {
    return [
      { nombre:'Auriculares Bluetooth TWS', score:82, margen:45, demanda:'alta', riesgo:'bajo', ticket:'$25.000', justificacion:'Alta demanda sostenida en MeLi. Buen margen de importación.', topPick:true },
      { nombre:'Cargador Inalámbrico 15W', score:76, margen:50, demanda:'alta', riesgo:'bajo', ticket:'$18.000', justificacion:'Producto de bajo costo FOB con alta rotación.', topPick:false },
      { nombre:'Smartwatch Deportivo', score:74, margen:40, demanda:'media', riesgo:'medio', ticket:'$45.000', justificacion:'Mercado en crecimiento pero con competencia.', topPick:false },
      { nombre:'Funda Silicona Premium', score:70, margen:55, demanda:'alta', riesgo:'bajo', ticket:'$8.000', justificacion:'Volumen masivo, bajo costo de entrada.', topPick:false },
      { nombre:'Soporte Auto Magnético', score:68, margen:48, demanda:'media', riesgo:'bajo', ticket:'$12.000', justificacion:'Accesorio de alta rotación en canales digitales.', topPick:false }
        ];
}
    ...
    try {
          const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
                  ...
                  body:JSON.stringify({
                            model:'llama-3.3-70b-versatile',
                            messages:[
                              { role:'system', content:'Sos un consultor especializado en importaciones China-Argentina y MercadoLibre. Respondés en español rioplatense, directo y útil.'+(context?` Contexto: ${context}`:'') },
        ...
        const data = await response.json();
          ...
          res.json({ reply:'Error al procesar la consulta.' });
...
app.listen(PORT, () => {
    ...
    console.log(`MELI_ACCESS_TOKEN: ${process.env.MELI_ACCESS_TOKEN ? 'CONFIGURADO' : 'FALTA CONFIGURAR'}`);
